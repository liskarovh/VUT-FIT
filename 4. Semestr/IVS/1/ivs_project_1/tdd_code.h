...
protected:
    ...
    //hashmap for nodes, size_t is id of node, Node is node structure
    //for lookup, insertion, deletion of nodes
    std::unordered_map<size_t, Node> node_map;

    //hashmap for edges, size_t is id of node, unordered_set is set of nodes that are connected to this node
    //for lookup, insertion, deletion of edges
    std::unordered_map<size_t, std::unordered_set<size_t>> adjacency_list;

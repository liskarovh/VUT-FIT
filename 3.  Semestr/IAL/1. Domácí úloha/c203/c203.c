...
void Queue_Init( Queue *queue ) {
	if (queue==NULL)
	{
		Queue_Error(QERR_INIT); //error
		return;
	}

	for (int i = 0; i < QUEUE_SIZE; i++) {
        queue->array[i] = '*';  // Nastavení veškerých hodnot na '*'
    }
	
	queue->firstIndex=0; //první prvek na pozici 0
	queue->freeIndex=0; //první volné místo na pozici 0
}
...
int nextIndex( int index ) {
	return (index + 1) % QUEUE_SIZE; //zaručení kruhovosti fronty
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	if (Queue_IsEmpty(queue)) //pokud je fronta prázdná, error
	{
 ...
 * @param queue Ukazatel na inicializovanou strukturu fronty
 */
void Queue_Remove( Queue *queue ) { //DODELAT
	...
	queue->firstIndex = nextIndex(queue->firstIndex); //přesunutí prvního indexu o 1 - zruší první hodnotu
	if (queue->firstIndex == QUEUE_SIZE)
	{
		queue->firstIndex = 0; //ošetření kruhovosti
	}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	...
	Queue_Front(queue, dataPtr); //zpřístupní první prvek fronty a uloží ho do dataPtr
	Queue_Remove(queue);
...
void Queue_Enqueue( Queue *queue, char data ) {
	if (Queue_IsFull(queue)) //fronta je již plná
	{
		Queue_Error(QERR_ENQUEUE);
		return;
	}
	
	queue->array[queue->freeIndex] = data; //vloží data na volný index
	queue->freeIndex = nextIndex(queue->freeIndex); //posune volný index
